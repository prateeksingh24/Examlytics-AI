    ...
    <div className="bg-white rounded-xl shadow-lg border border-indigo-100 overflow-hidden flex flex-col h-full relative w-full print:h-auto print:border-none print:shadow-none print:overflow-visible print:block">
      <div className="bg-gradient-to-r from-indigo-600 to-violet-600 px-4 sm:px-6 py-5 flex items-center justify-between shrink-0 print:bg-none print:text-black">
         ...
         {analysisText && !loading && (
             <button 
                onClick={handleShare} 
                className={`p-2 rounded-lg backdrop-blur-sm transition-all flex items-center gap-2 text-sm font-medium print:hidden ${copied ? 'bg-green-500/90 text-white' : 'bg-white/20 hover:bg-white/30 text-white'}`}
                title="Share Analysis"
             >
                {copied ? (
                    <><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg><span className="hidden sm:inline">Copied</span></>
                ) : (
      ...
      <div className="p-4 sm:p-6 bg-gray-50/50 min-h-[550px] overflow-y-auto custom-scrollbar flex-grow print:overflow-visible print:h-auto print:bg-white">
        {loading ? (
          <div className="flex flex-col items-center justify-center h-full space-y-6 py-12">
            <div className="relative">
                <div className="w-16 h-16 border-4 border-indigo-100 border-t-indigo-600 rounded-full animate-spin"></div>
                <div className="absolute inset-0 flex items-center justify-center"><div className="w-8 h-8 bg-indigo-50 rounded-full"></div></div>
            </div>
            <div className="text-center space-y-2 px-4">
                <p className="text-indigo-900 font-bold text-lg">Analyzing Performance & Generating Plan...</p>
                <p className="text-gray-500 text-sm max-w-xs mx-auto">Identifying weak spots, time sinks, and creating your personalized study schedule.</p>
            </div>
          </div>
        ) : (
            <>
                    ...
                    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <div className="space-y-6">
                            <div className="flex items-center gap-4 pb-2 border-b border-gray-200">
                                ...
                                <h3 className="text-xl font-bold text-gray-800">Performance Report Analysis</h3>
                            </div>
                        ...
                        {planText && (
                            <div className="space-y-6">
                                <div className="flex items-center gap-4 pb-2 border-b border-gray-200">
                                    ...
                                    <h3 className="text-xl font-bold text-gray-800">1-Week Personalized Study Plan</h3>
                                </div>
